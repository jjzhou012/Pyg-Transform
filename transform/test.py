...
from utils.utils import get_topK, remove_outEdge_from_center
...
class Generate_malfunctioning_exchange_account(BaseTransform):
    ...
    def __call__(self, data):
        ...
        # batch = data.batch if 'batch' in data else None
        if data.y:
            edge_index, edge_attr = remove_outEdge_from_center(edge_index=data.edge_index, edge_attr=data.edge_attr, num_nodes=data.x.size(0))
        else:
            return data
